...
func TestXADD(t *testing.T) {
	...
	t.Run("accepts strictly increasing IDs 1-1 then 1-2", func(t *testing.T) {
		resetStores()
		got1, err := DispatchCommand(req("XADD", "stream_key", "1-1", "foo", "bar"))
		if err != nil {
			t.Fatal(err)
		}
		...
		got2, err := DispatchCommand(req("XADD", "stream_key", "1-2", "bar", "baz"))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("rejects ID equal to top entry", func(t *testing.T) {
		...
		got, err := DispatchCommand(req("XADD", "stream_key", "1-2", "baz", "foo"))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("rejects ID with smaller time than top even if sequence is larger", func(t *testing.T) {
		resetStores()
		mustDispatch(t, req("XADD", "stream_key", "1-1", "foo", "bar"))
		got, err := DispatchCommand(req("XADD", "stream_key", "0-3", "baz", "foo"))
		if err != nil {
			t.Fatal(err)
		}
		want := resp.WriteError("ERR The ID specified in XADD is equal or smaller than the target stream top item")
	...
	t.Run("rejects 0-0 on empty stream", func(t *testing.T) {
		resetStores()
		got, err := DispatchCommand(req("XADD", "stream_key", "0-0", "baz", "foo"))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("asterisk auto id returns bulk string ms-seq", func(t *testing.T) {
		resetStores()
		got, err := DispatchCommand(req("XADD", "stream_key", "*", "foo", "bar"))
		if err != nil {
			t.Fatal(err)
		}
		id := mustReadBulkString(t, got)
		parsed, ok := utils.ParseStreamID(id)
		if !ok {
	...
	t.Run("consecutive asterisk XADD ids are strictly increasing", func(t *testing.T) {
		resetStores()
		got1, err := DispatchCommand(req("XADD", "stream_key", "*", "a", "1"))
		if err != nil {
			t.Fatal(err)
		}
		got2, err := DispatchCommand(req("XADD", "stream_key", "*", "b", "2"))
		if err != nil {
			t.Fatal(err)
		}
		id1, ok1 := utils.ParseStreamID(mustReadBulkString(t, got1))
		id2, ok2 := utils.ParseStreamID(mustReadBulkString(t, got2))
		if !ok1 || !ok2 {
			t.Fatal("parse id")
		}
	...
	t.Run("asterisk after explicit id generates id greater than last", func(t *testing.T) {
		...
		autoID, ok := utils.ParseStreamID(mustReadBulkString(t, got))
		lastID, ok2 := utils.ParseStreamID("9999999999999-42")
		if !ok || !ok2 {
			t.Fatal("parse")
		}
	...
	t.Run("partial seq 1-* first entry is 1-0", func(t *testing.T) {
		resetStores()
		got, err := DispatchCommand(req("XADD", "some_key", "1-*", "foo", "bar"))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("partial seq 1-* second entry increments sequence", func(t *testing.T) {
		...
		got, err := DispatchCommand(req("XADD", "some_key", "1-*", "bar", "baz"))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("partial seq rejects ms smaller than top entry", func(t *testing.T) {
		...
		got, err := DispatchCommand(req("XADD", "some_key", "1-*", "a", "b"))
		if err != nil {
			t.Fatal(err)
		}
		want := resp.WriteError("ERR The ID specified in XADD is equal or smaller than the target stream top item")
	...
	t.Run("partial seq 0-* on empty stream is 0-1", func(t *testing.T) {
		resetStores()
		got, err := DispatchCommand(req("XADD", "z", "0-*", "x", "y"))
		if err != nil {
			t.Fatal(err)
		}
