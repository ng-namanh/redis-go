...
```json metadata
{
  ...
  "group": "stream",
  "command_flags": ["readonly","blocking","movablekeys"],
  "acl_categories": ["@read","@stream","@slow","@blocking"],
  "since": "5.0.0",
  "arity": -4,
  "key_specs": [{"RO":true,"access":true,"begin_search":{"spec":{"keyword":"STREAMS","startfrom":1},"type":"keyword"},"find_keys":{"spec":{"keystep":1,"lastkey":-1,"limit":2},"type":"range"}}],
  "tableOfContents": {"sections":[{"id":"non-blocking-usage","title":"Non-blocking usage"},{"id":"incomplete-ids","title":"Incomplete IDs"},{"id":"blocking-for-data","title":"Blocking for data"},{"id":"the-special-id","title":"The special $ ID."},{"id":"the-special-id","title":"The special + ID"},{"id":"how-multiple-clients-blocked-on-a-single-stream-are-served","title":"How multiple clients blocked on a single stream are served"},{"id":"redis-software-and-redis-cloud-compatibility","title":"Redis Software and Redis Cloud compatibility"},{"id":"return-information","title":"Return information"}]}
...
fashion to [`BRPOP`](https://redis.io/docs/latest/commands/brpop) or [`BZPOPMIN`](https://redis.io/docs/latest/commands/bzpopmin) and others.

Please note that before reading this page, if you are new to streams,
we recommend to read [our introduction to Redis Streams](https://redis.io/docs/latest/develop/data-types/streams).

## Non-blocking usage

If the **BLOCK** option is not used, the command is synchronous, and can
be considered somewhat related to [`XRANGE`](https://redis.io/docs/latest/commands/xrange): it will return a range of items
inside streams, however it has two fundamental differences compared to [`XRANGE`](https://redis.io/docs/latest/commands/xrange)
...
* While [`XRANGE`](https://redis.io/docs/latest/commands/xrange) returns items in a range of IDs, `XREAD` is more suited in
...
To use incomplete IDs is valid, like it is valid for [`XRANGE`](https://redis.io/docs/latest/commands/xrange). However
...
producers of data to use [`XADD`](https://redis.io/docs/latest/commands/xadd) to push new entries inside the streams
...
to the stream via [`XADD`](https://redis.io/docs/latest/commands/xadd) starting from the moment we block. In such a case
...
client waiting will be served as soon as an [`XADD`](https://redis.io/docs/latest/commands/xadd) command provides
data to the stream.

Reading the [Redis Streams introduction](https://redis.io/docs/latest/develop/data-types/streams) is highly
