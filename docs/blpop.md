...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"non-blocking-behavior","title":"Non-blocking behavior"},{"id":"blocking-behavior","title":"Blocking behavior"},{"id":"what-key-is-served-first-what-client-what-element-priority-ordering-details","title":"What key is served first? What client? What element? Priority ordering details."},{"id":"behavior-of-blpop-when-multiple-elements-are-pushed-inside-a-list","title":"Behavior of BLPOP when multiple elements are pushed inside a list."},{"id":"blpop-inside-a-multi-exec-transaction","title":"BLPOP inside a MULTI / EXEC transaction"},{"id":"examples","title":"Examples"},{"id":"reliable-queues","title":"Reliable queues"},{"id":"pattern-event-notification","title":"Pattern: Event notification"},{"id":"redis-software-and-redis-cloud-compatibility","title":"Redis Software and Redis Cloud compatibility"},{"id":"return-information","title":"Return information"}]}
...
It is the blocking version of [`LPOP`](https://redis.io/docs/latest/commands/lpop) because it blocks the connection when there
...
client performs an [`LPUSH`](https://redis.io/docs/latest/commands/lpush) or [`RPUSH`](https://redis.io/docs/latest/commands/rpush) operation against one of the keys.
...
* After an [`EXEC`](https://redis.io/docs/latest/commands/exec) of a [`MULTI`](https://redis.io/docs/latest/commands/multi) block with multiple push operations against the same list.
...
If the above condition happens using a Redis 2.6 server or greater, Client **A** will be served with the `c` element, because after the [`LPUSH`](https://redis.io/docs/latest/commands/lpush) command the list contains `c,b,a`, so taking an element from the left means to return `c`.
...
Using `BLPOP` inside a [`MULTI`](https://redis.io/docs/latest/commands/multi) / [`EXEC`](https://redis.io/docs/latest/commands/exec) block does not make a lot of sense
as it would require blocking the entire server in order to execute the block
atomically, which in turn does not allow other clients to perform a push
operation. For this reason the behavior of `BLPOP` inside [`MULTI`](https://redis.io/docs/latest/commands/multi) / [`EXEC`](https://redis.io/docs/latest/commands/exec) when the list is empty is to return a `nil` multi-bulk reply, which is the same
...
[`MULTI`](https://redis.io/docs/latest/commands/multi) / [`EXEC`](https://redis.io/docs/latest/commands/exec) block...
...
This can be a problem with some application where we want a more reliable messaging system. When this is the case, please check the [`BRPOPLPUSH`](https://redis.io/docs/latest/commands/brpoplpush) command, that is a variant of `BLPOP` that adds the returned element to a target list before returning it to the client.
...
This would require a blocking version of [`SPOP`](https://redis.io/docs/latest/commands/spop) that is not available, but using
